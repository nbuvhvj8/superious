...
export default function OnboardingPage() {
  ...
  function handleGoogleConnect() {
    ...
    if (!clientId) {
      setGoogleStatus('error');
      setErrorMsg('Google Client ID is not configured. Please add NEXT_PUBLIC_GOOGLE_CLIENT_ID to your environment variables.');
      return;
    }

    const redirectUri = `${process.env.NEXT_PUBLIC_SITE_URL}/api/auth/google/callback`;
    const scope = encodeURIComponent('https://www.googleapis.com/auth/documents https://www.googleapis.com/auth/drive.file email profile');
      ...
      <div className="flex items-center gap-2 mb-10">
        {STEPS?.map((s, i) => (
          <React.Fragment key={s?.id}>
            ...
            {i < STEPS?.length - 1 && (
              <div className={`w-8 h-px ${i < step ? 'bg-primary' : 'bg-border'} transition-colors duration-300`} />
            )}
          </React.Fragment>
        ))}
      </div>
        ...
        {step === 0 && (
          <div className="card p-8 space-y-6 animate-fade-in text-center">
            ...
            <div className="grid grid-cols-2 gap-3 text-left">
              {[
                { icon: <Sparkles size={14} />, label: 'AI Script Generation' },
                ...
                <div key={f?.label} className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-muted">
        ...
        {step === 1 && (
          <div className="card p-8 space-y-6 animate-fade-in">
            <div className="space-y-1">
              ...
              <h2 className="text-xl font-extrabold text-foreground">Everything you need to create</h2>
            </div>
            <div className="space-y-3">
              {[
                ...
                <div key={f?.title} className="flex items-start gap-3 p-3.5 rounded-xl bg-muted/60 border border-border">
        ...
        {step === 2 && (
          <div className="card p-8 space-y-6 animate-fade-in">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">
                Connect your Google account to export scripts directly to Google Docs. You can skip this and connect later in Settings.
              </p>
            </div>
            ...
            <div className="flex gap-3">
              ...
              {googleStatus === 'connected' ? (
                ...
                <button onClick={handleSkipGoogle} className="btn-ghost flex-1 text-muted-foreground">
                  Skip for now
                </button>
              )}
            </div>
          ...
          <div className="card p-8 space-y-6 animate-fade-in text-center">
            ...
            {googleStatus !== 'connected' && (
              <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-muted border border-border text-left">
                ...
                <p className="text-xs text-muted-foreground">
                  Google Docs export is not connected. Go to <strong>Settings → Integrations</strong> to connect later.
                </p>
              </div>
            )}
